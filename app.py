...
def index():
    ...
    db = get_db()
    question_cur = db.execute('''select questions.id as question_id, questions.question_text, askers.name as asker_name, experts.name as expert_name 
                                 from questions 
...
def users():
    ...
    db = get_db()
    user_cur = db.execute('select id, name, password, admin from users')
